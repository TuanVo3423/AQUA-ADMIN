...
export default function InputAdornments() {
  ...
  <div style={{width : '70%' , margin : 'auto' , textAlign : 'center'}}>
    <Typography variant='h2' component={"h2"}>FORM CREATE PRODUCT</Typography>
  <Box sx={{ display: 'flex', flexDirection : 'column', marginTop : '20px' ,flexWrap: 'wrap' , width : '100%' , gap : '30px' }}>
   <TextField onChange={((e) => setValues({...values, name : e.target.value}))} fullWidth label="NAME" id="fullWidth" />
   ...
   <TextField value={values.shortDescription} onChange={((e) => setValues({...values, shortDescription : e.target.value}))} fullWidth label="SHORTDESCRIPTION" id="fullWidth" />
   <TextField value={values.detailDescription} onChange={((e) => setValues({...values, detailDescription : e.target.value}))} multiline rows={5} fullWidth label="DETAILDESCRIPTION" id="fullWidth" />
   <TextField value={values.price} onChange={((e) => setValues({...values, price : e.target.value}))} fullWidth label="PRICE" id="fullWidth" />
   ...
   <Button onClick={handleSubMit} sx={{width : '50%' , display : 'flex' ,  margin : 'auto' , padding : '10px 0'}} variant='contained'>SUBMIT</Button>
    </Box>
  </div>
  );
}
